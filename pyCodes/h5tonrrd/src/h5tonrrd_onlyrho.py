...
# values using user defined reference values
    ...
    ref_var = {'rho_b' : 1.225, # bulk density (kg/m^3)
    'T_w' : 300.0, # wall temperature (K)
    'h' : 0.1 # channel half height (m)
    } 
    const_var = {'gamma' : 1.4, # specific heat ratio
                 'R_gas' : 287.0  # specific gas constant (J/kg.K)
                 } 
    ref_var['V'] = np.sqrt(const_var['gamma'] * const_var['R_gas'] * ref_var['T_w'])

    # calculate scaling values for the variables
    scale_var = {'rho' : np.float(ref_var['rho_b']), 'u' : np.float(ref_var['V']), 
