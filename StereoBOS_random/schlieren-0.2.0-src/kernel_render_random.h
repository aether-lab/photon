...
__global__ void kernel_render(float3* ray_pos, float3* ray_dir, RenderParameters* paramsp, float4* inout_pixels, unsigned int* out_pixels, LightField* lightfield, float* random_array_theta, float* random_array_phi, unsigned int* source_pixels,int *launchCount,int* receiveCount, float* offsetCount)
{
   ...
   pos_c = make_float3(0,0,-0.5);   // co-ordinates of the center of the dot card or BOS texture in physical space
	...
	//*************************************** Generate a cone of light rays ********************************************
        ...
        float R = 1.0, phi_cone = 5/180*M_PI; // phi_cone is cone half angle in radians
        float del_phi = 0.5/180*M_PI, del_theta = 60.0/180*M_PI;	// angular interval (in radians) between two rays
	...
	*/
	float thresh = 0.1; // threshold for angle in degrees
        float theta, phi;
        //float x, y, z;
	float angle;
	int flag = 0;
	...
	for(int j = 0;j<paramsp->random_array_size;j++)
	{
	  phi = 0.5*M_PI*random_array_phi[j];
	  theta = 2*M_PI*random_array_theta[j]; 
